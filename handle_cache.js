...
function generateFiles(floderPath, obj, cb) {
	Promise.all([
		tranferEN(floderPath, obj),
		tranferZH(floderPath, obj),
		tranferTW(floderPath, obj)
	])
		.then(function() {
...
function tranferZH(floderPath, obj) {
...
function tranferTW(floderPath, obj) {
	return new Promise(async function(resolve, reject) {
		...
		mkdir(floderPath + '/tw', async function() {
			...
			for (var key in obj) {
				all.push(
					`"${key.split('.')[1]}":"${opencc.simplifiedToTraditional(obj[key])}",`
				)
			}
...
function tranferEN(floderPath, obj) {
	return new Promise(function(resolve, reject) {
		// 生成英文资源
